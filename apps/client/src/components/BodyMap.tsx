...
import { BODY_REGION_LABELS, type BodyRegion } from '@zophiel/shared';
...
// Each zone is a <path> or <ellipse> traced over the body silhouette
...
export default function BodyMap({ selectedRegion, onSelectRegion, painIntensityLevel }: BodyMapProps) {
  ...
  const getZoneClass = (zoneId: BodyRegion) => {
    ...
    if (selectedRegion === zoneId) {
      cls += ' selected';
      if (painIntensityLevel) cls += ` pain-${painIntensityLevel}`;
    }
    return cls;
  };
  ...
  return (
    <div className="body-map-container">
      ...
      <div className="body-svg-wrapper">
        <svg viewBox="0 0 200 470" xmlns="http://www.w3.org/2000/svg">
          {/* Background glow */}
          <defs>
          ...
          {zones.map((zone) => (
            <path
              ...
              filter={selectedRegion === zone.id ? 'url(#zoneShadow)' : undefined}
            >
